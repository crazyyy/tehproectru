...
function initMapAndLayers(mainLayer){
	...
	if (extentByIp){
		initExtent = new esri.geometry.Extent(extentByIp.xmin, extentByIp.ymin, extentByIp.xmax, extentByIp.ymax, (mainLayer) ? (mainLayer.initialExtent.spatialReference) : (new esri.SpatialReference({"wkid": 102113	})));
	}
	else
		if (mainLayer)
			initExtent = mainLayer.initialExtent;
		else
			initExtent = new esri.geometry.Extent(3000000, 5000000, 15000000, 11000000, new esri.SpatialReference({
				"wkid": 102113
			}));
	...
	_mapLayers["map"] = esri.layers.ArcGISTiledMapServiceLayer(LAYERS_URL.map, { tileServers: [LAYERS_URL.map_a, LAYERS_URL.map_b, LAYERS_URL.map_c] });
	_mapLayers["map"].hide();
	...
	_terrZoneLayers["select"] = new esri.layers.ArcGISDynamicMapServiceLayer(LAYERS_URL.terrZoneSelect, {opacity: 0.45});
	...
	_zouitLayers["select"] = new esri.layers.ArcGISDynamicMapServiceLayer(LAYERS_URL.zouitSelect, {opacity: 0.45});
    ...
    _atdLayers["select"] = new esri.layers.ArcGISDynamicMapServiceLayer(LAYERS_URL.atdSelect, {opacity: 0.45});
    ...
    _frameLayers["select"] = new esri.layers.ArcGISDynamicMapServiceLayer(LAYERS_URL.frameSelect, {opacity: 0.45});
	...
	_mapLayers["anno"] = new esri.layers.ArcGISTiledMapServiceLayer(LAYERS_URL.annotation, { tileServers: [LAYERS_URL.annotation_a, LAYERS_URL.annotation_b, LAYERS_URL.annotation_c] });
